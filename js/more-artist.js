...
document.addEventListener("DOMContentLoaded", function () {
  const posterGrid = document.querySelector("#artist .portfolio-wrap");
  const posters = Array.from(posterGrid.children).filter(child => child.classList.contains('portfolio'));
  ...
  function initializeAndShowFirstRow() {
    ...
    for (let i = 0; i < initialVisibleCount && i < posters.length; i++) {
      posters[i].style.display = "flex"; // 또는 'flex', 'grid' 등 원래 display 속성
    }
